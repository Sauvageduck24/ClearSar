...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Pipeline YOLO ClearSAR: train + inference"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="960",
        help="Tamaño de imagen para train/inference (ej: 960 o [512,1024]).",
    )

    # --- Misc (wait-gpu, cache, evolve)
...
def main() -> None:
    ...
    effective_image_size = str(yolo_train_image_size if yolo_train_image_size is not None else args.image_size)
    ...
    if not args.skip_inference:
        ...
        if not _has_arg(yolo_infer_tokens, "--image-size"):
            yolo_infer_cmd.extend(["--image-size", effective_image_size])
